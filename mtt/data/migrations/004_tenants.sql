...
CREATE USER IF NOT EXISTS tenant_duff_svc   TYPE = SERVICE LOGIN_NAME = 'TENANT_DUFF_SVC'
  DEFAULT_ROLE = tenant_duff_viewer   DEFAULT_WAREHOUSE = mtt_wh DEFAULT_NAMESPACE = 'MTT_DB.SERVING';
CREATE USER IF NOT EXISTS tenant_krusty_svc TYPE = SERVICE LOGIN_NAME = 'TENANT_KRUSTY_SVC'
  DEFAULT_ROLE = tenant_krusty_viewer DEFAULT_WAREHOUSE = mtt_wh DEFAULT_NAMESPACE = 'MTT_DB.SERVING';
...
BEGIN
  FOR i IN 0 TO ARRAY_SIZE(:roles)-1 DO
    ...
    EXECUTE IMMEDIATE 'GRANT USAGE ON DATABASE mtt_db TO ROLE ' || :r;
    EXECUTE IMMEDIATE 'GRANT USAGE ON SCHEMA mtt_db.serving TO ROLE ' || :r;
    EXECUTE IMMEDIATE 'GRANT SELECT ON VIEW mtt_db.serving.sales TO ROLE ' || :r;
    EXECUTE IMMEDIATE 'GRANT USAGE ON WAREHOUSE mtt_wh TO ROLE ' || :r;
  END FOR;
END;
$$;
