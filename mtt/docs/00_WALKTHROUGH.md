...
- **`001_init.sql`** — creates the `mtt_admin` role and grants it to
`CURRENT_USER()`. This role owns the data model.
- **`002_objects.sql`** — creates the `mtt_db` database, the shared `mtt_wh`
...
it, creating the `EXTERNAL_OAUTH` security integration `mtt_keycloak`. Two
mappings matter:
...
This creates the `mtt-web` confidential client — the web app's
...
warehouse, four tenant roles, two service users, `mtt_admin`, and the
`mtt_keycloak` integration. The generated `auth/clients.json` and `auth/web.env`
